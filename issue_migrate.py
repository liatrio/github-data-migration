...
source_enterprise = Github(base_url=os.getenv("GHES_URL"), auth=source_auth)
dest_enterprise = Github(auth=dest_auth)

# create a function to filter assignees and add it to the body the issue
def filter_assignees(source_repo, assignees, body):
  assignees_list = []
  body = body +  "\n\n**Assignees:**" if body else "**Assignees:**" if assignees else ""
  for assignee in assignees:
    try:
      source_repo.get_user(assignee.login)
...
def migrate_issues(source_name, dest_name):
  ...
  source_repo = source_enterprise.get_repo(source_name)
  dest_repo = dest_enterprise.get_repo(dest_name)
  ...
  for issue in source_issues:
    try:
      filtered_assignees, body = filter_assignees(source_repo, issue.assignees, issue.body)
